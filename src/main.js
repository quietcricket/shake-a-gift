...
class ShakeAGift {
   ...
   */
  share() {
    let message = this.shakeCount >= CONFIG.PRIZE_UNLOCK ? CONFIG.SUCCESS_TWEET : CONFIG.FAILURE_TWEET;
...
class Utils {
  ...
  webpPolyfill() {
    ...
    document.querySelectorAll(".gif").forEach(ele => {
      ...
      ele.style.left = (document.querySelector(".section").offsetWidth - parseInt(ele.getAttribute("img-width"))) / 3.2 + "px";
    });
  }
